...
class Categories extends Component
{
                ...
                if (this.state.name.trim().length > 3 && this.state.description.trim().length > 3)
                {
                    this.setState({...this.state, isCreating: false}, () =>
                    {
                        setTimeout(() => this.setState({...this.state, loading: true}, () =>
                        {
                            ...
                            fetch('http://185.211.58.174:1435/category/create', {
                                method: 'POST',
                                headers: {
                                ...
                                .then((responseJson) =>
                                {
                                    if (responseJson.state === 1)
                                    {
                                        ...
                                        fetch(`http://185.211.58.174:1435/category/${id}`, {
                                            headers: {
                                                'Cache-Control': 'no-cache',
                                            },
                                        })
                ...
                if (this.state.name.trim().length > 3 && this.state.description.trim().length > 3)
                {
                    this.setState({...this.state, isCreating: false}, () =>
                    {
                        setTimeout(() => this.setState({...this.state, loading: true}, () =>
                        {
                            ...
                            fetch('http://185.211.58.174:1435/category/update', {
                                method: 'POST',
                                headers: {
                                    'Cache-Control': 'no-cache',
                                    'auth': 'QEFiZWxtaXJpIEBIb3NleW5tb3VzYXZp',
                                ...
                                .then((responseJson) =>
                                {
                                    if (responseJson.state === 1)
                                    {
                                        fetch(`http://185.211.58.174:1435/category/${this.state.updateId}`, {
                                            headers: {
                                                'Cache-Control': 'no-cache',
                                            },
                                        })
    ...
    handleDelete(id, name)
    {
        ...
        if (submit)
        {
            ...
            {
                fetch('http://185.211.58.174:1435/category/delete', {
                    method: 'post',
                    headers: {
